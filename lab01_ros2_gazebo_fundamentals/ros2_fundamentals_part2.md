...
Use the [WSL/Ubuntu Terminal shortcuts from Lab 00](../lab00_setup/README.md#wslubuntu-terminal-shortcuts) to complete long paths with `Tab` and recall earlier build or inspection commands with the arrow keys or `Ctrl+R`.
...
A **package** is one organized unit of ROS 2 software. A **workspace** is the directory in which one or more packages are developed and built.

| Concept | Meaning | Example in this practice |
|---|---|---|
| package | related nodes, launch files, configuration, metadata, and dependency declarations | `eel4332_ros_practice` |
| workspace | a build area containing packages under `src/` and the results generated by `colcon` | `~/eel4332_ws` |

A useful analogy is that a package is one project, while the workspace is the workshop where one or more projects are built together. After Practice 4, the layout will look approximately like this:

```text
~/eel4332_ws/                         ← workspace
├── src/
│   └── eel4332_ros_practice          ← package link
├── build/                            ← intermediate build files
├── install/                          ← installed, runnable packages
└── log/                              ← build records
```

The package link points to the provided package inside your course repository, so you edit one copy of the source while `colcon` still finds it under the workspace's `src/` directory.

The distinction also appears in the commands you use:

- Run `colcon build` from the **workspace root**; it finds and builds packages under `src/`.
- Source the **workspace** with `source ~/eel4332_ws/install/setup.bash`; this makes its built packages visible in the current terminal.
- Run a particular executable by giving `ros2 run` its **package name** and **executable name**, such as `ros2 run eel4332_ros_practice counter_publisher`.
